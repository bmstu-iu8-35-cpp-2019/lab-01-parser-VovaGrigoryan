...
void Json::parse_object(const std::string &s, int &a, int &b) {
    ...
    while (true) {
        ...
        a = b + 1;
        b = static_cast<int>(
                s.substr(a).find_first_of("{[\"0123456789tfn-") + a);
        ...
        switch (s[b]) {
            ...
            case '-': {
                ...
                if (static_cast<int>(
                    ...
                    if (static_cast<int>(s.substr
                    (a + 1, b - a).find_first_of("+-")) !=
                        -1) {
                        int c = static_cast<int>(
                                s.substr(
                                a + 1, b - a).find_first_of("+-") + a + 1);
                        if (!(s[c - 1] == 'e' || s[c - 1] == 'E'))
                            throw std::exception();
                    }
                    if (static_cast<int>(s.substr
                    (a + 1, b - a).find_first_of("eE")) !=
                        -1) {
                        int c = static_cast<int>(
                                s.substr(a + 1, b - a).
                                find_first_of("eE") + a + 1);
                        if (static_cast<int>(s.substr(c + 1, b - c).
                        find_first_of("eE")) !=
                            -1)
                            throw std::exception();
                    }
...
void Json::parse_array(const std::string &s, int &a, int &b) {
    ...
    while (true) {
        std::any value = nullptr;
        a = b + 1;
        b = static_cast<int>(s.substr(a).
                find_first_of("{[\"0123456789tfn-") + a);
        if (static_cast<int>(s.substr(a).
                find_first_of("{[\"0123456789tfn-")) == -1) {
            if (vector.empty()) {
                b = s.substr(a).find(']') + a;
                check_whitespace(s.substr(a, b - a));
                break;
...
bool Json::is_object() const {
    if (json_map.type() ==
        typeid(std::unordered_multimap<std::string, std::any>))
        return true;
    else
        return false;
}
...
std::any Json::operator[](const std::string &key) {
    if (this->is_object()) {
        auto object =
                std::any_cast<std::unordered_multimap
                        <std::string, std::any>>(json_map);
